...
async def vnc(e):
    ...
    try:
        ...
        LOGS.exception(er)
        return await xxx.edit("`فشل التحويل في الصوت...`")
    await xxx.delete()
    os.remove(file.name)
    os.remove("out.opus")


@ultroid_cmd(pattern="تحميل بصمه( (.*)|$)")
...
@ultroid_cmd(pattern="استخراج البصمه$")
async def ex_aud(e):
