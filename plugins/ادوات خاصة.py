...
async def hbd(event):
    ...
    if dan == 0:
        ...
        okk = 365 + ish
        hp = f"{okk} ايام متبقية 🥳"
    elif dan > 0:
        hp = f"{ish} ايام متبقية 🥳"
    if month == "01":
...
يسكن -:  {saal}yr, {mahina}m, {din}d, {ghanta}hr, {mi}min, {slive}sec

عيد الميلاد -: {hp}

الأبراج الفلكية -: {sign}
...
@ultroid_cmd(pattern="صورة عالية( (.*)|$)")
