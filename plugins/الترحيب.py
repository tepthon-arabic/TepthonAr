...
@ultroid_cmd(pattern="وضع ترحيب", groups_only=True)
...
@ultroid_cmd(pattern="ترحيب واضح$", groups_only=True)
...
@ultroid_cmd(pattern="الترحيبات$", groups_only=True)
async def listwel(event):
    ...
    await event.reply(f"**الترحيبات في هذه الدردشة**\n\n`{msgg}`", file=med)
    await event.delete()


@ultroid_cmd(pattern="وضع ترحيب المغادره", groups_only=True)
...
@ultroid_cmd(pattern="ترحيب مغادره واضح$", groups_only=True)
...
@ultroid_cmd(pattern="ترحيبات المغادره$", groups_only=True)
async def listgd(event):
    ...
    if wel.get("button"):
        btn = create_tl_btn(wel["button"])
        return await something(event, msgg, med, btn)
    await event.reply(f"**ترحيبات المغادره في هذه المحادثه**\n\n`{msgg}`", file=med)
    await event.delete()


@ultroid_cmd(pattern="شكر الأعضاء (تفعيل|تعطيل)", groups_only=True)
async def thank_set(event):
    type_ = event.pattern_match.group(1).strip()
    if not type_ or type_ == "":
        await eor(
            event,
            f"**إعدادات الدردشة الحالية:**\n**شكر الأعضاء:** `{must_thank(event.chat_id)}`\n\nUse `{HNDLR}thankmembers on` or `{HNDLR}thankmembers off` to toggle current settings!",
        )
        return
    chat = event.chat_id
    if type_.lower() == "on":
    ...
    await eor(
        event,
        f"**تم! شكرا لك الأعضاء قد فرحت** `{type_.lower()}` **for this chat**!",
    )
