...
@ultroid_cmd(pattern="تثبيت$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
        return await eor(msg, get_string("pin_1"))
    me = await msg.get_reply_message()
    if me.is_private:
        text = "`تم تثبيت الرسالة بنجاح.`"
    else:
        text = f"الرسالة تم تثبيتها [رابط الرسالة]({me.message_link}) !"
    try:
        await msg.client.pin_message(msg.chat_id, me.id, notify=False)
    except BadRequestError:
        return await eor(msg, get_string("adm_2"))
    except Exception as e:
        return await eor(msg, f"**خطا:**`{e}`")
    await eor(msg, text)
...
async def unp(ult):
    ...
    try:
        ...
        return await xx.edit(f"**خطا:**`{e}`")
    await xx.edit("`تم الغاء تثبيت الرسالة!`")


@ultroid_cmd(
    pattern="تثبيت في الشات( (.*)|$)",
    admins_only=True,
    manager=True,
    require="pin_messages",
    fullsudo=True,
...
async def pin_message(ult):
    ...
    if not ult.is_reply:
        return await ult.eor("`قم بالرد على الرسالة..`", time=6)
    if not match:
        return await ult.eor("`خطأ حاول في وقت لاحق..`", time=8)
...
@ultroid_cmd(pattern="مسح( (.*)|$)", manager=True, require="delete_messages")
...
@ultroid_cmd(pattern="حظر الكل", manager=True, groups_only=True)
...
async def autodelte(ult):
    ...
    try:
        ...
        return await ult.eor(
            f"المسح التلقائي شغال بالفعل!`{match}`", time=5
        )
    await ult.eor(f"تم تفعيل مسح الرسائل التلقائي `{match}` !")
