...
@ultroid_cmd(
    pattern="النقر التلقائي (تشغيل|تعطيل)$",
    admins_only=True,
    manager=True,
    require="ban_users",
    fullsudo=True,
...
@ultroid_cmd(pattern="حظر القناة التلقائي$", admins_only=True)
async def ban_cha(ult):
    if autoban_db.is_autoban_enabled(ult.chat_id):
        autoban_db.del_channel(ult.chat_id)
        return await ult.eor("`تعطيل Auto ChannelBan ♥ ️🧸...`")
    if not (
        ult.chat.creator
    ...
    await ult.eor("`تم تمكين حظر القناة التلقائي بنجاح ♥️🧸!`")
...
@ultroid_cmd(pattern="(قائمة|الاضافة|rem)wl( (.*)|$)")
async def do_magic(event):
    ...
    if not usea:
        return await Msg.edit(
            "`يرجى تقديم اسم مستخدم للقناة/معرف لإضافة / إزالة إلى / من القائمة البيضاء 🧸♥️..`"
        )
    try:
    ...
    if match == "add":
        ...
        return await msg.edit(f"`يضيف` {inline_mention(cha)} `إلى القائمة البيضاء..`")
