...
• `{i}هدم <reply to animated sticker>`
    لتدمير الملصق.

• `{i}صغير جدا <reply to media>`
    لإنشاء ملصقات صغيرة.

• `{i}كانغ <reply to image/sticker>`
    كانغ الملصق (أضف إلى حقيبتك).

• `{i}باككانغ <pack name>`
    مجموعة ملصقات كانغ الكاملة (باسم مخصص).

• `{i}مستدير <reply to any media>`
    لاستخراج الملصق الدائري.
"""
import glob
import io
import os
...
async def hehe(args):
    ...
    if photo:
        ...
        if (
            ...
            await xx.edit("`تخمير عبوة جديدة 🤍🧸...`")
            async with ultroid_bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                await conv.send_message(packnick)
