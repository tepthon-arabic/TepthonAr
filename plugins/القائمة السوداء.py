...
@ultroid_cmd(pattern="القائمة السوداء( (.*)|$)", admins_only=True)
async def af(e):
...
@ultroid_cmd(pattern="حذف القائمة السوداء( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@ultroid_cmd(pattern="قائمة سوداء$", admins_only=True)
async def lsnote(e):
