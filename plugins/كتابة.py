...
• `{i}اكتب <text or reply to text>`
   سوف يكتب على ورقة.

• `{i}صورة <text or reply to html or any doc file>`
   اكتب صورة من لغة تأشير النص الفائق أو أي نص.
"""
...
@ultroid_cmd(pattern="احصل على html( (.*)|$)")
async def ghtml(e):
...
async def f2i(e):
    ...
    if txt:
        ...
        if r.media:
            ...
            html = r.text
    if not html:
        return await eod(e, "`إما الرد على أي ملف أو إعطاء أي نص ♥ ️🧸`")
    html = html.replace("\n", "<br>")
