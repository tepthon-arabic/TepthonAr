...
async def echo(e):
    ...
    if r:
        ...
        try:
            ...
            return await e.eor("يجب عليك الرد على الاسم المستخدم ♥️🧸.", time=5)
...
async def rm(e):
    ...
    if r:
        ...
        try:
            ...
            return await e.eor("يجب الرد على الاسم المستخدم ♥️🧸.", time=5)
    if check_echo(e.chat_id, user):
...
@ultroid_cmd(pattern="قائمة المتحكمين$")
async def lstecho(e):
    if k := list_echo(e.chat_id):
        user = "**قائمة المتحكمين 🧸♥️:**\n\n"
        for x in k:
